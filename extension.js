...
{
  const disposable =
    vscode.commands.registerCommand('mdheader.addHeader', () => {
      ...
      const header_arr =
        _CreateHeader(filename, project_name, file_date, comment);
...
function _CreateHeader(filename, project_name, file_date, comment)
{
  ...
  const comment_start =
    (comment.lineComment) ? comment.lineComment : comment.blockComment[0];
  const comment_end =
    (comment.lineComment) ? comment.lineComment : comment.blockComment[1];
  const comment_len = (comment_start.length + comment_end.length);

  const MAX_COLUMNS = 80;
  const border_line =
    comment_start + '-'.repeat(MAX_COLUMNS - comment_len) + comment_end;
